...
class BpmnProcess:
    ...
    def start_process(self,bpmn_diagram,handler=None,context={}):
    ...
    def _execute_start_event(self):
        start_event = StartEvent(self._get_start_task_event(), self)
    ...
    def get_activity_for_outgoing(self,activity_type,activity_id,activity_data):
        ...
        if(activity_type == "scriptTask"):
            activity = ScriptTask(activity_data,self)
            self.activities.append(activity)  

        if(activity_type == "manualTask"):
            activity = ManualTask(activity_data,self)
            self.activities.append(activity)

        if(activity_type == "serviceTask"):
        ...
        if(activity_type == "exclusiveGateway"):
            activity = ExclusiveGateway(activity_data,self)
            self.activities.append(activity)

        if(activity_type == "inclusiveGateway"):
            activity = InclusiveGateway(activity_data,self)
            self.activities.append(activity)

        if(activity_type == "parallelGateway"):
    ...
    def _get_start_task_event(self):
        tmp_nodes = self.diagram.get_nodes()
